...
if [ -f "$SERVER_CONFIG_PATH" ]; then
	...
	sed -i "s/^ServerName =.*/ServerName $(printf '%s\n' "$SERVER_NAME" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^MaxPlayerCount =.*/MaxPlayerCount $(printf '%s\n' "$PLAYERS" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^WelcomeMessage =.*/WelcomeMessage $(printf '%s\n' "$MOTD" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^ProbyAdminPassword =.*/ProbyAdminPassword $(printf '%s\n' "$PROBY_PASS" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
  sed -i "s/^SeniorAdminPassword =.*/SeniorAdminPassword $(printf '%s\n' "$SENIOR_PASS" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^AdminPassword =.*/AdminPassword $(printf '%s\n' "$ADMIN_PASS" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
  sed -i "s/^WhiteListIsEnabled =.*/WhiteListIsEnabled $(printf '%s\n' "$WHITELIST" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^#\{0,1\}ServerPassword =.*/ServerPassword $(printf '%s\n' "$SERVER_PASS" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^PortID =.*/PortID $(printf '%s\n' "$SERVER_PORT" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
  sed -i "s/^SteamGamePortID =.*/SteamGamePortID $(printf '%s\n' "$STEAM_GAME_PORT" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
	sed -i "s/^SteamQueryPortID =.*/SteamQueryPortID $(printf '%s\n' "$STEAM_QUERY_PORT" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
  sed -i "s/^SteamPortID =.*/SteamPortID $(printf '%s\n' "$STEAM_PORT" | sed -e 's/[\/&]/\\&/g')/g" "$SERVER_CONFIG_PATH"
else
